...
1. **先读 3 篇，建立心智模型**：
   - `docs/4-决策层/系统简介.md`（定位与目标）
   - `docs/1-生产层/开发文档/00-项目概述与架构概览.md`（五层架构）
   - `docs/1-生产层/modules/README.md`（模块树与完成度）
...
| 3 | `docs/1-生产层/modules/README.md:4` | 包数量 27 → 32（internal/）+ 全仓库 36 口径说明 |
| 4 | `docs/4-决策层/系统简介.md:30` | 32 包加 scope；ONNX 召回 1.00 加「默认=TF-IDF 降级」说明 |
| 5 | `docs/4-决策层/版本发布说明.md:28/29/32` | 语义检索/影响面分析加生效边界；包数加 scope |
| 6 | `DEV_PROGRESS.md:235` | Dockerfile 基础镜像 alpine → `golang:1.25-bookworm → debian:bookworm-slim` |
| 7 | `docs/1-生产层/开发文档/11-配置部署与路线图.md:365` | 同上，alpine → bookworm |
