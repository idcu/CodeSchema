...
func TestFilterDocs(t *testing.T) {
	...
	tt := []struct {
		...
		},
		{
			...
			where:         nil,
			whereDocument: map[string]string{WhereContains: "llo"},
			want:          []*Document{docs["1"], docs["2"]},
		},
		{
			name:          "content contains one",
			where:         nil,
			whereDocument: map[string]string{WhereContains: "hallo"},
			want:          []*Document{docs["2"]},
		},
		{
			name:          "content contains none",
			where:         nil,
			whereDocument: map[string]string{WhereContains: "bonjour"},
			want:          nil,
		},
		{
