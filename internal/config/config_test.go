...
func TestYAML_InlineList(t *testing.T) {
	parsed, err := parseYAML(`
languages: [go, java, rust]
`)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	list, ok := parsed["languages"].([]any)
	if !ok {
		t.Fatalf("expected list, got %T", parsed["languages"])
	}
	if len(list) != 3 {
		t.Errorf("expected 3 items, got %d", len(list))
	}
	if list[0] != "go" || list[1] != "java" || list[2] != "rust" {
...
`)
	...
	if !ok {
		t.Fatalf("expected map for storage, got %T", parsed["storage"])
	}
	if storage["driver"] != "sqlite" {
		t.Errorf("storage.driver = %v, want sqlite", storage["driver"])
	}
	if storage["dsn"] != "/data/db.sqlite" {
		t.Errorf("storage.dsn = %v, want /data/db.sqlite", storage["dsn"])
	}

	vector, ok := storage["vector"].(map[string]any)
	if !ok {
		t.Fatalf("expected map for vector, got %T", storage["vector"])
	}
	if vector["driver"] != "chromem" {
		t.Errorf("vector.driver = %v, want chromem", vector["driver"])
	}
}
...
`)
	...
	if !ok {
		t.Fatalf("expected map, got %T", parsed["project"])
	}
	if proj["name"] != "test-repo" {
		t.Errorf("project.name = %v, want test-repo", proj["name"])
	}
	if proj["root"] != "/test" {
		t.Errorf("project.root = %v, want /test", proj["root"])
	}
}

func TestYAML_BoolAndNumber(t *testing.T) {
	parsed, err := parseYAML(`
enabled: true
count: 42
pi: 3.14
...
`)
	...
	if parsed["enabled"] != true {
		t.Errorf("enabled = %v (%T), want true", parsed["enabled"], parsed["enabled"])
	}
	if parsed["count"] != 42 {
		t.Errorf("count = %v (%T), want 42", parsed["count"], parsed["count"])
	}
	if parsed["disabled"] != false {
		t.Errorf("disabled = %v (%T), want false", parsed["disabled"], parsed["disabled"])
	}
}
...
func TestYAML_CommentsOnly(t *testing.T) {
	parsed, err := parseYAML(`
# This is a comment
# Another comment
# 
...
func TestParseValue_InlineList(t *testing.T) {
	v := parseValue("[go, java, rust]")
	list, ok := v.([]any)
	if !ok {
...
func TestParseValue_EmptyInlineList(t *testing.T) {
	v := parseValue("[]")
	list, ok := v.([]any)
	if !ok {
		t.Fatalf("expected []any, got %T", v)
...
func TestParseValue_Number(t *testing.T) {
	...
	f, ok := v.(float64)
	if !ok || f != 3.14 {
		t.Errorf("expected 3.14, got %v (%T)", v, v)
	}
}
