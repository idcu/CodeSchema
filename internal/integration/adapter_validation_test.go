// Package integration 提供跨适配器（SCIP / LSP）的生产验证与多语言基准测试。
//
...
//
// 运行：go test -run TestAdapterValidation ./internal/integration/ -v
package integration
...
func TestAdapterValidation(t *testing.T) {
	...
	_ = os.WriteFile(filepath.Join("build", "adapter-bench.json"), data, 0o644)
	t.Logf("Adapter validation report:\n%s", string(data))

	// 输出 Markdown 摘要到 analysis/
...
func validateLSPGo(t *testing.T) adapterResult {
	...
	path := writeSample(t, filepath.Join(t.TempDir(), "go"), "svc.go", src)

	a := lsp.NewGoplsAdapter()
...
func validateLSPCpp(t *testing.T) adapterResult {
	...
	src := "class Service {\npublic:\n    void Run();\n    void Stop();\n};\n"
	path := writeSample(t, filepath.Join(t.TempDir(), "cpp"), "svc.cpp", src)
...
func validateLSPJava(t *testing.T) adapterResult {
	...
	path := writeSample(t, filepath.Join(t.TempDir(), "java"), "Service.java", src)
...
func writeAdapterMarkdown(t *testing.T, results []adapterResult) {
	...
	_ = os.WriteFile(filepath.Join(dir, "2026-08-14-adapter-validation.md"), []byte(b.String()), 0o644)
}
