...
6. **开发文档索引**：`docs/1-生产层/开发文档/` 实际含 `00`–`13` 共 14 篇；README「开发指南」与本文均已全部列出（`12` 于 2026-08-15 补入、`13` 于 2026-08-16 补入）。
...
### 文档
- [x] `docs/1-生产层/开发文档/` — 14 个开发文档按开发顺序分割（`00`–`13`）
- [x] `DEV_PROGRESS.md` — 本文件，开发进度跟踪
- [x] `docs/1-生产层/modules/` — 模块级文档（P1~P9 分层拆解，43 份）
...
- [x] **优先级② SCIP / LSP 适配器生产验证** —
  ...
  > **2026-08-15 更新**：clangd 场景已补齐工程上下文真实验证（`TestLSPAdapter_RealClangd` 构造 compile_commands.json 最小工程，clangd 22 真实提取类/方法 PASS），并修复其根因——`jsonRPCRequest.ID` 缺 `omitempty` 使 notification 携带 `"id":0` 违反 JSON-RPC 2.0，clangd 严格拒绝（gopls 宽容未暴露）。详见 `docs/1-生产层/modules/P3_4.md`。
...
- [x] **优先级④ 超大仓瓶颈验证 + PG/Redis 迁移路径** —
  ...
  - **文档** `docs/1-生产层/开发文档/12-存储扩展与大规模迁移路径.md`：回填实测表格 + 修正结论（SQLite 实为超线性主导瓶颈）+ 迁移路径（SQLite+BulkUpsert / chromem 持久化 / PG 横向 / Redis 缓存）。
...
- [x] **T2-5 配置模板与一键体验**（`6ebb93a`）— `codeschema mcp --print-config` 输出 5 类客户端配置 + `docs/3-使用层/客户端接入指南（MCP）.md` + README 快速开始
...
多项目都需要 CodeSchema 时，落地「单实例多租户」方案（设计见 `docs/1-生产层/开发文档/13-多租户设计文档.md`，可运行示例 `build/mt-demo.yaml`）：
...
- [x] **文档** — 新增 `docs/1-生产层/开发文档/13-多租户设计文档.md`；更新 `README.md`/`docs/1-生产层/开发文档/11`/`docs/3-使用层/客户端接入指南（MCP）.md`/`docker-compose.yml`（新增 `codeschema-mt` 服务，`--profile mt`）。
...
1. 阅读 `docs/1-生产层/开发文档/00-项目概述与架构概览.md` 了解整体架构
2. 按 `docs/1-生产层/开发文档/` 编号顺序阅读对应开发文档
